...
from pywmi.install import check_installation_xadd
...
pytestmark = pytest.mark.skipif(
    check_installation_xadd(), reason="XADD engine is not installed"
)
...
def test_normalization():
    ...
    for i in range(5):
        ...
        assert (
            RejectionEngine(
                domain, ~Iff(support, clean_support), Real(1.0), 1000000
            ).compute_volume()
            == 0
        )
        assert (
            RejectionEngine(
                domain, ~Iff(new_support, clean_new_support), Real(1.0), 1000000
            ).compute_volume()
            == 0
        )
        ...
        illegal_volume = XaddEngine(
            domain, ~new_support, new_weight, "mass"
        ).compute_volume()
        assert illegal_volume == pytest.approx(0, rel=EXACT_REL_ERROR)

        computed_volume = XaddEngine(
            domain, TRUE(), new_weight, "mass"
        ).compute_volume()
        computed_volume_within = XaddEngine(
            domain, new_support, new_weight, "mass"
        ).compute_volume()
        computed_volume_within2 = XaddEngine(
            domain, new_support, new_weight
        ).compute_volume()
        computed_volume_within3 = RejectionEngine(
            domain, new_support, new_weight, 1000000
        ).compute_volume()
        print(
        ...
        assert computed_volume_within == pytest.approx(
            computed_volume_within2, EXACT_REL_ERROR
        )
        print(
            "pa true new_weight:",
            computed_volume,
        ...
        illegal_volume = engine.copy_with(
            support=~new_support, weight=new_weight
        ).compute_volume()
        assert illegal_volume == pytest.approx(0, rel=EXACT_REL_ERROR)
...
def test_normalization_negative():
    ...
    pa_engine = RejectionEngine(
        domain, Iff(new_support, ~normalize_formula(new_support)), Real(1), 1000000
    )
...
def test_xadd_iff_real():
    ...
    xadd_vol1 = XaddEngine(
        domain, (f1 | f2) & (~f1 | ~f2), smt.Real(1.0)
    ).compute_volume()
