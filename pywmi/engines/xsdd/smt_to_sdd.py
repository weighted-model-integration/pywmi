...
class PySmtConversion(Semiring):
    def __init__(self, literals, env=None):
        super().__init__()
        self.env = get_env() if env is None else env
        self.fm = self.env.formula_manager
        self.reverse_varnums = {num: var for num, var in literals.numbered().items()}

    def times_neutral(self):
        return self.fm.TRUE()

    def plus_neutral(self):
        return self.fm.FALSE()

    def times(self, a, b, index=None):
        return self.fm.And(a, b)

    def plus(self, a, b, index=None):
        return self.fm.Or(a, b)

    def negate(self, a):
        return self.fm.Not(a)

    def positive_weight(self, a):
        return self.fm.Symbol(self.reverse_varnums[a], BOOL)
