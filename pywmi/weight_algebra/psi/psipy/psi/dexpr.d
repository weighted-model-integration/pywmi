...
abstract class DExpr{
	...
	final freeVars(){
		static struct FreeVars{ // TODO: move to dutil?
			DExpr self;
			DExprSet visited;
			@disable this(this);
...
bool mustBeZeroOrOne(DExpr e){
	...
	if(auto m=cast(DMult)e) return dutil.all!mustBeZeroOrOne(m.factors);
	return false;
}
...
DExpr cancelFractions(bool isDelta)(DExpr e,DIvr.Type type=DIvr.Type.eqZ){
	...
	if(!simple&&(dutil.among(type,DIvr.Type.eqZ,DIvr.Type.neqZ))&&(e.getFractionalFactor().c<0||nege.getFractionalFactor().c>=0&&nege.isMoreCanonicalThan(e))){
...
in{static if(is(T==DIvr)) with(DIvr.Type) assert(dutil.among(cond.type,eqZ,neqZ,leZ));}body{
...
class DIvr: DExpr{ // iverson brackets
	...
	@conditionallyEven!((Type t,DExpr e)=>dutil.among(t,DIvr.Type.eqZ,DIvr.Type.neqZ)) DExpr e;
	mixin Visitors;
	...
	static DExpr staticSimplify(Type type,DExpr e,DExpr facts=one){
		...
		}else if(dutil.among(type,Type.eqZ,Type.neqZ)){
			if(auto m=cast(DMult)e){
				DExprSet nf;
...
template BitwiseImpl(string which){
	...
	override DExpr simplifyImpl(DExpr facts){
		...
		static if(which=="bitOr"||which=="bitAnd"){
			if(dutil.all!(x=>mustBeZeroOrOne(x))(operands)){
				static if(which=="bitOr"){
					DExprSet ops;
