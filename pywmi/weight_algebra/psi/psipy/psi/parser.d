...
import lexer, error, dutil, expression, type, declaration;
...
template rbp(TokenType type){enum rbp=type==Tok!"."?180:lbp!type-(lbp!type==30||dutil.among(type,Tok!"^",Tok!"?",Tok!"->",Tok!"→",Tok!"⇒",Tok!"↦",Tok!"=>"));}
...
struct Parser{
		...
		switch(ttype){
			...
			case Tok!"(",Tok!"[":
				if(allowLambda){
					...
					while(dutil.among(ttype,Tok!"(",Tok!"[")){
						nextToken();
						skipToUnmatched();
						nextToken();
					}
	...
	FunctionDef parseFunctionDef(bool lambda=false,bool semicolon=!lambda)(){
		...
		CompoundExp body_;
		if(dutil.among(ttype,Tok!"⇒",Tok!"↦",Tok!"=>",Tok!"(",Tok!"[")){
			Expression e;
			if(ttype==Tok!"("||ttype==Tok!"["){
