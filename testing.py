...
def diabetes_example1():
    ...
    support = ((f0 & (((d0 | ~d0) & ~(r0 <= 35)) | ((r0 <= 35) & (~d0)))) | (~f0 & (d0 & (r0 <= 35)))) & domain.get_bounds()
    ...
    support_smaller = ((f0 & ~(r0 <= 35) & ~d0) | (f0 & (r0 <= 35) & ~d0)) & domain.get_bounds()
    ...
    weight_function_smaller = smt.Real(0.0001) * r0 *\
                              smt.Real(0.001) * r0 * r0 #* smt.Real(1000)  #<--- add this changes the result from 0.0 to 102
    density = Density(domain, support_smaller & domain.get_bounds(), weight_function_smaller) #weight_function)
    query = d0

    startTime = time.time()
    rejection_engine = RejectionEngine(density.domain, density.support, density.weight, sample_count=1000000)
    ...
    mfxsdd = FXSDD(density.domain, density.support, density.weight,
                   vtree_strategy=balanced, algebra=algebra, ordered=False)
    startTime = time.time()
